...
suspend fun maiSongAlias(event:GroupMessageEvent,regexList:MutableList<String>){
    val songList=MaiSongList()
    for(aliasRaw in regexList){
        val alias=aliasRaw.substring(0,aliasRaw.length-4)
        ...
        if(data.length()==0){
            ...
            for(i in 0 until data.length()){
                ...
                if(temp.getString("alias").lowercase()==alias.lowercase()){
                    ...
                    send(event,MessageChainBuilder()
                        .append("您要找的是不是：\n")
            ...
            if(data.length()>=4){
                ...
                for(i in data.toList().indices){resultString+=songList.id(data.getJSONObject(i)
                    .getInt("musicId")).songInfoSummary.endl()}
