...
class MaiB40Result {
    ...
    inner class MaiSongResult(val json: JSONObject, val num: Int) {
        ...
        fun drawSong(): Image? {
            ...
            val bg = setLight(
                GaussianBlurUtil.blur(
                    resizeImage(ImageIO.read(song!!.getImageFile()), 200, 200), 10
                ), -80
            )
    ...
    fun drawB40Image(): File {
        ...
        g.drawString(
            "Rating: $rating (底分) + $additionalRating (段位分) = ${rating + additionalRating} " +
